...
module.exports = async() => {
  ...
  const updatedNameObj = await getProperties.updateName( pluginProps.name, nameObj );
  templates.copyTemplates( updatedNameObj, pluginProps );
  ...
  if ( pluginProps.admin ) {
    ...
    status.start();
    await installNpmDeps( updatedNameObj.base );
    status.stop();
  ...
  if ( repoSpecs.git_repo ) {
    ...
    if ( files.directoryExists( '.git' ) ) {
      ...
      // 8b. Create .gitignore
      await repo.createGitignore( updatedNameObj.kebab );
  ...
  // 9. Success message
  success( updatedNameObj, pluginDir );
};