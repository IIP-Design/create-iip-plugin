...
module.exports = async() => {
  ...
  if ( repoSpecs.git_repo ) {
    ...
    if ( files.directoryExists( '.git' ) ) {
      ...
      let getRepo;
      const done = await repo.setUpRepo( pluginDir, pluginProps.uri, getRepo );
      if( done ) {
        const validRemote = await checkOrigin.checkRepo( pluginProps.uri );
        console.log( chalk.green( '\n Repo initialized!' ) );
        if ( validRemote ) {
          console.log( chalk.dim( ` Origin set to ${pluginProps.uri}. Don't forget to create the equivalent GitHub repo.` ) );
        } else {
